...
public class ItemHandler {
    ...
    public Mono<ServerResponse> getAllItem(ServerRequest serverRequest) {
        return ServerResponse.ok()
                ...
                .body(Flux.just(
                        new ItemModel(UUID.randomUUID().toString(), "Name1", 10, Collections.emptyList()),
                        new ItemModel(UUID.randomUUID().toString(), "Name2", 20, List.of(
                                new SubItemModel("SubModel1", BigDecimal.valueOf(1001)),
                                new SubItemModel("SubModel2", BigDecimal.valueOf(2002))
                        ))), ItemModel.class);
    }
